...
为保证历史成本的准确性，系统使用独立的日志文件记录价格和倍率变化历史：

- `~/.claude/pricing_history.log` - 模型定价历史
- `~/.claude/upstream_rate_history.log` - 渠道倍率历史

**pricing_history.log格式**:
```
# 格式: datetime,model,input,output,cache_creation,cache_read
2025-12-28 14:21:00,claude-opus-4-5-20251101,5.0,25.0,6.25,0.5
```

**upstream_rate_history.log格式**:
```
# 格式: datetime,upstream,rate
2025-12-28 14:21:00,2api,0.6
```
